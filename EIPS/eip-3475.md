...
`classValues(uint256 classId)` allows anyone to read the information of a bond class.

The `"classId"` is the class of the bond, the first bond class created will be 0, and so on.
...
`classDescriptions(uint256 classInfo)` allows anyone to read the information description of a class info given.
...
`nonceValues(uint256 classId, uint256 nonceId)` allows anyone to read the values of a bond class' nonce.
...
```
function nonceDescriptions(uint256 classId, uint256 nonceId) external view returns (uint256[] memory);
```

**NonceDescriptions**

`nonceDescriptions(uint256 classId, uint256 nonceInfo)` allows anyone to read the information description of a nonce info given.

The `"nonceDescriptions"` is the descriptions of the nonce values 

Returns the nonce descriptions of the nonce. *Every bond contract can have their own list of info description.*

```
nonceDescriptions(uint256 classId, uint256 nonceInfo) external view returns (string[] memory);
```
