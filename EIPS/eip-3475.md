...
This example shows the transfer from `_from` address, to `_to` address , with `500000000` bonds  of  type class`1` and  nonce `42` (as defined in the transaction array datastructure).
...
`approve(address _spender, TRANSACTION[] calldata _transaction)` Allows _spender to withdraw from the msg.sender  the bonds of _amount and type (classId and nonceId).
