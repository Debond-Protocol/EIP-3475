...
`approve(address _spender, uint256 classId, uint256 nonceId, uint256 _amount)` Allows spender to withdraw from your account multiple times, up to the amount.
...
`setApprovalFor(address _operator, uint256 classId, bool _approved)` Enable or disable approval for a third party ("operator") to manage Bond class of the caller's bonds.
