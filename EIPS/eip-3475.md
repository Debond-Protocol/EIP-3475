...
```
**Batch function**: ERC 3475 supports batch operations such as batch transaction. It allows the user to transfer different bonds along with their  metadata to a new address instantaneously in single txn . This mechanism helps in the "packaging" of bonds which is useful in usecases  like  package of bonds trades on a secondary market, the new owner holds the right to reclaim the face value of each of the bonds.
...
To ensure the reading of transaction, `"Issued"` ,`"Redeemed"` ,`"Burned"` ,`"Transfer"`, Events cited above MUST be emitted when such transaction is passed. 
