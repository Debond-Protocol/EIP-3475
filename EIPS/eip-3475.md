...
**NOTE: the conditions of redemption can be speechified with one or several internal functions.**
...
`function allowance(address _owner, address _spender, uint256 classId, uint256 nonceId, uint _amount) external view returns (uint256)` authorises to set the allowance and also returning  the same  for given class classId and  nonceId Iff the address ("_operator") is approved to manage account's Bonds class.
...
### Metadata: 
The Metadata of a bond class or nonce are stored as an array of JSON objects, taking an example : 
...
also for example  there can be possibility to have localisation of the issuance / redemption of the bond, by describing the jurisdiction code (fr, de, cn rtc) as follows:
```json
{
"title": "localisation",
"type": "string",
...
The integer in the values array can be regarded as the keys to the values stored as JSON object.
...
[ERC3475 reference implementation](https://github.com/Debond-Protocol/EIP-3475/tree/update-EIP/contracts).
