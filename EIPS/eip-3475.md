...
```Solidity
function redeem(0x2d03B6C79B75eE7aB35298878D05fe36DC1fE8Ef, [{1,42,500000000}]);
```
...
``` Solidity
function burnBond(0x82a55a613429Aeb3D01fbE6841bE1AcA4fFD5b2B,[{1,42,500000000}]);
```
...
```Solidity
function approve(0x82a55a613429Aeb3D01fbE6841bE1AcA4fFD5b2B,[{0,1,30000}]);
```
...
Returns an array of uint256 values of a bond nonce. — e.g. `[1615584000,...]`.*** The metadata SHOULD follow a set of structures explained later in this document .*
...
And then override  the function interface for functions `classValues` and `nonceValues` by defining the key (which SHOULD be a index) to read the corresponding information stored as a JSON object. 
e.g.
...
```
 In the above example, to get the `“symbol"` of the given class id, we can use the class id as key to get the `“symbol"` value in the JSON metadata `"symbol 3"`, which then can be used for fetching the detail for instance. 
...
`"Issue"` MUST be triggered when the operator (Bank) contract issues bonds to the given entity.
