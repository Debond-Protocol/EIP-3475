...
`classValues(uint256 classId, uint256 metadataId)` allows anyone to read the values (stored in struct METADATA for different bondNonces) for given bond class `classId`.
...
`nonceValues(uint256 classId, uint256 nonceId, uint256 metadataId)` allows anyone to read the values of a bond nonce.
