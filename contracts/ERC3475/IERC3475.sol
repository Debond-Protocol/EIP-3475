...
interface IERC3475 {
    ...
    /**
        ...
        The `_class` argument MUST be the class of the token being transferred.
    ...
    */
    event TransferSingle(address indexed _operator, address indexed _from, address indexed _to, uint256 _class, uint256 _nonce, uint256 _amount);


    /**
        @dev Either `TransferSingle` or `TransferBatch` MUST emit when tokens are transferred, including zero value transfers as well as minting or burning (see "Safe Transfer Rules" section of the standard).
    */
    event TransferBatch(address indexed _operator, address indexed _from, address indexed _to, uint256[] _classes, uint256[] _nonces, uint256[] _amounts);
    ...
    */
     ...
     */
    function isRedeemable(uint256 _class, uint256 _nonce) external view returns (bool);
    ...
    function batchIsApprovedFor(address account, address operator, uint256[] calldata _classes, uint256[] calldata _nonces) external view returns (bool);

    /**
     * @dev Transfer of any number of bond types from an address to another.
     */
    function batchTransferFrom(address _from, address _to, uint256[] calldata _classes, uint256[] calldata _nonces, uint256[] calldata _amount) external;
