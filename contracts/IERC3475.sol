...
interface IERC3475 {
     ...
     */
    function batchTransferFrom(address _from, address _to, uint256[] calldata _classIds, uint256[] calldata _nonces, uint256[] calldata _amount) external;
    ...
    */
    function setApprovalFor(address _operator, uint256[] calldata classIds, uint256[] calldata nonces, bool _approved) external;
    ...
    */
    ...
    /**
        @dev Either `TransferSingle` or `TransferBatch` MUST emit when tokens are transferred, including zero value transfers as well as minting or burning (see "Safe Transfer Rules" section of the standard).
    */
    event TransferBatch(address indexed _operator, address indexed _from, address indexed _to, uint256[] _classIds, uint256[] _nonces, uint256[] _amounts);

    /**
        @dev MUST emit when approval for a second party/operator address to manage all tokens for an owner address is enabled or disabled (absence of an event assumes disabled).
    */
    event ApprovalFor(address indexed _owner, address indexed _operator, uint256[] classId, uint256[] nonce, bool _approved);
