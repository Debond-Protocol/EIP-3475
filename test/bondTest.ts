...
contract('Bond', async (accounts: string[]) => {
    ...
    it('should return bond classIds', async () => {
        ...
        const classIds = (await bondContract.getClasses()).map(classId => classId.toNumber())
        assert.isArray(classIds);
        assert.equal(classIds[0], 0);
    })

    it('should issue bonds to the given address for a classId and nonceId given', async () => {
        await bondContract.issue(lender, 0, 0, 1000, {from: accounts[0]})
